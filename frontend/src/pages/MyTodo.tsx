...
import { api, Project, StepItem, Steps } from '../api/client';
...
interface Row { project: Project; stage: string; item: StepItem; isCurrent: boolean; forConfirm: boolean }

/** 青、灰身份的首页：只有“轮到我做什么”；多数动作原地完成。 */
export default function MyTodo() {
  ...
  const load = useCallback(async () => {
    ...
    for (const p of projects) {
      const st = await api.steps(p.id).catch(() => null);
      if (!st) continue;
      const curIdx = st.current_stage.key === 'done' ? st.stages.length : st.stages.findIndex((s) => s.key === st.current_stage.key);
      for (const [si, s] of st.stages.entries()) {
        // 在建：不列未来段；已售 portfolio：仍列未完成收尾（关水电等）
        if (p.stage !== 'portfolio' && si > curIdx) continue;
        for (const it of s.items) {
          if (it.done) continue;
          const mineTick = it.owners.includes(role.actor);
          const mineConfirm = needsMyConfirm(it, role.actor);
          if (mineTick || mineConfirm) {
            all.push({
              project: p,
              stage: s.label,
              item: it,
              isCurrent: p.stage === 'portfolio' ? false : s.key === st.current_stage.key,
              forConfirm: mineConfirm && (it.gate || !mineTick),
            });
          }
        }
      }
    }
    all.sort((a, b) => Number(b.isCurrent) - Number(a.isCurrent));
    setRows(all);
  }, [role.actor]);
  ...
  const openAction = (row: Row) => {
    const canDo = row.forConfirm ? true : canActOn(row.item, role);
    const mode = actionMode(row.item, { forConfirm: row.forConfirm });
    if (mode === 'navigate' || mode === 'view' || (!canDo && mode !== 'confirm')) {
      navigate(actionHref(row.project.id, row.item, { forConfirm: row.forConfirm }));
      return;
    }
    if (mode === 'tick') {
  ...
  const doTick = async () => {
    ...
    setBusy(true);
    try {
      await api.toggleStep(modal.row.project.id, modal.row.item.key, { done: true });
      await afterDone(modal.row.item.title, modal.row.project.id);
    } catch (e: any) {
  ...
  const doConfirm = async (confirmAs: string) => {
    ...
    setBusy(true);
    try {
      await api.toggleStep(modal.row.project.id, modal.row.item.key, { done: true, confirm_as: confirmAs });
      await afterDone(modal.row.item.title, modal.row.project.id);
    } catch (e: any) {
      flash({ type: 'error', content: e.message });
    } finally {
  ...
  const saveField = async () => {
    ...
    try {
      ...
      await api.patchProject(modal.row.project.id, { [f]: f === 'purchase_price' ? Number(fieldVal) : fieldVal || null });
      await afterDone(modal.row.item.title, modal.row.project.id);
    } catch (e: any) {
      flash({ type: 'error', content: e.message });
    } finally {
  ...
  return (
    <ContentLayout header={<Header variant="h1" description={`你是 ${role.actor}（${role.tierLabel}）。${role.duties}`}>我的待办</Header>}>
      <Table
        loading={rows === null}
        loadingText="正在看哪套房轮到你"
        items={rows ?? []}
        header={<Header variant="h2" counter={rows ? `(${rows.length})` : undefined} description={current.length ? `其中 ${current.length} 件是现在这段的事，其余是前面段落没交的。` : '暂时没有轮到你的事。'}><ReviewTag id="A" />轮到我做的</Header>}
        empty={<Box textAlign="center" padding="l">暂时没有轮到你的事。</Box>}
        columnDefinitions={[
          { id: 'p', header: '哪套房', cell: (r) => <div><div style={{ fontWeight: 700 }}>{r.project.name}</div><Box variant="small" color="text-body-secondary">{r.project.property.address_std}</Box></div> },
          { id: 's', header: '阶段', cell: (r) => <span>{r.stage}{r.isCurrent && <Box variant="span" color="text-status-info">　现在这段</Box>}{r.project.stage === 'portfolio' && <Box variant="span" color="text-body-secondary">　已售收尾</Box>}</span> },
          ...
          {
            id: 'a', header: '', width: 140, minWidth: 140,
            cell: (r) => {
              const canDo = r.forConfirm || canActOn(r.item, role);
              const label = actionLabel(r.item, { actor: role.actor, canDo, forConfirm: r.forConfirm });
              return (
      ...
      />
      <Box margin={{ top: 'm' }}><SpaceBetween size="xs"><Box variant="small" color="text-body-secondary">看不到金额和预算是正常的，这些只对决策和统筹级别显示。</Box></SpaceBetween></Box>
      ...
      >
        {modal?.kind === 'upload' && (
          <UploadForm
            projectId={modal.row.project.id}
            ...
            compact
            onDone={() => { afterDone(modal.row.item.title, modal.row.project.id); }}
          />
        )}
      </Modal>
      ...
      >
        {modal?.kind === 'confirm' && (
          <SpaceBetween size="m">
            ...
            <Box variant="small" color="text-body-secondary">{modal.row.project.name} · {modal.row.stage}</Box>
